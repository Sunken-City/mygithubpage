...
<body>
  ...
  <!-- pako is a fallback for gzip if the browser lacks CompressionStream -->
...
.iconbtn{
  width:var(--tap); min-width:var(--tap); padding:0; display:grid; place-items:center;
}
...
# --- icon.svg (emoji-only-ish, no binaries) ---
Write-Utf8NoBom (Join-Path $dir "icon.svg") @'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256">
  <rect width="256" height="256" rx="48" fill="#0b0f19"/>
  <text x="128" y="150" text-anchor="middle" font-size="120" font-family="system-ui,Segoe UI,Apple Color Emoji">💪</text>
</svg>
'@
...
(() => {
  ...
  function defaultSave() {
    const t = now();
    const skills = [
      { name:"Upper Front Strength", icon:"💪", color:"#f97316" },
      { name:"Upper Back Strength", icon:"🧱", color:"#60a5fa" },
      { name:"Lower Front Strength", icon:"🦵", color:"#22c55e" },
      { name:"Lower Back Strength", icon:"🏋️", color:"#a78bfa" },
      { name:"Walking", icon:"🚶", color:"#38bdf8" },
      { name:"Nutrition", icon:"🥗", color:"#84cc16" },
      { name:"Portion Control", icon:"🍽️", color:"#f59e0b" },
      { name:"Sleep", icon:"😴", color:"#94a3b8" },
      { name:"Mobility", icon:"🧘", color:"#fb7185" },
      { name:"Hydration", icon:"💧", color:"#0ea5e9" }
    ].map((s, i) => ({
      id: createId(),
      name: s.name,
      icon: s.icon,
    ...
    el.innerHTML = `
      <div class="modal" role="dialog" aria-modal="true">
        <div class="modalHead">
          ...
          <button class="iconbtn" data-modal-close aria-label="Close">✕</button>
        </div>
  ...
  function homeView() {
    ...
    const reminder = (() => {
      ...
      return `<div class="card" style="border-color:color-mix(in srgb, var(--warn) 55%, var(--border)); box-shadow:none">
        <div class="row"><div class="stack" style="gap:4px">
          <div class="h2">Backup reminder</div>
          <div class="small">It’s been ${days} days since your last backup.</div>
        </div>
        <button class="btn-primary" data-nav="settings">Backup</button></div>
      </div>`;
    })();
    ...
    return `
      <div class="stack">
        ...
        <div class="card">
          ...
          ${qa.length === 0 ? `<p class="small" style="margin-top:10px">Create actions like “Gym session” → +10 Upper Front Strength, +5 Walking.</p>` : `
            <div class="stack" style="margin-top:10px">
              ${qa.map(a => `
                <div class="card" style="box-shadow:none">
                  <div class="row">
                    <div class="stack" style="gap:2px">
                      <div class="h2">${esc(a.icon||"⚡")} ${esc(a.name)}</div>
        ...
        <div class="row">
          <div class="row" style="justify-content:flex-start; gap:10px">
            ...
            <div class="stack" style="gap:2px">
              ...
              <div class="small">Level ${fmtInt(levelFromXp(s.xp))} • XP ${fmtXp(s.xp)}</div>
            </div>
          </div>
    ...
    return `
      <div class="stack">
        <div class="card">
          ...
          <p class="small" style="margin:8px 0 0">Desktop: drag cards when reorder is on. Mobile: use ↑/↓ on each card.</p>
        </div>
    ...
    return `
      <div class="stack">
        <div class="card" style="${s.color?`border-color:${esc(s.color)}55`:``}">
          <div class="row">
            <div class="stack" style="gap:4px">
              <h1 class="h1">${esc(s.icon||"⭐")} ${esc(s.name)}</h1>
              ...
              ${recent.map(e => {
                ...
                return `
                  <div class="card" style="box-shadow:none">
                    <div class="row">
                      <div class="stack" style="gap:2px">
                        <div class="h2">${e.type === "action" ? "⚡ Action" : "⭐ Skill XP"} • ${d?.delta>=0?"+":""}${fmtXp(d?.delta||0)}</div>
        ...
        <div class="card">
          ...
          <div class="grid grid2" style="margin-top:12px">
            ...
            <div class="stack" style="gap:6px">
              ...
              <input class="input" data-action-icon value="${icon}" placeholder="🏋️" />
            </div>
          </div>
                ...
                <div class="grid grid2" style="gap:10px">
                  <div class="stack" style="gap:6px">
                    ...
                    <select class="input" data-grant-skill="${idx}">
                      ${skills.map(s => `<option value="${esc(s.id)}" ${s.id===g.skillId?"selected":""}>${esc(s.icon||"⭐")} ${esc(s.name)}</option>`).join("")}
                    </select>
                  </div>
                  <div class="stack" style="gap:6px">
            ...
            ${events.map(e => `
              <div class="card">
                <div class="row">
                  <div class="stack" style="gap:4px">
                    <div class="h2">
                      ${e.type === "action" ? `⚡ ${esc(actionName(e.actionId))}` : `⭐ ${esc(skillName(e.skillId))}`}
    ...
    return `
      <div class="shell">
        ...
        <nav class="nav" aria-label="Bottom navigation">
          ${navBtn("home","🏠","Home",activeTab)}
          ${navBtn("skills","📜","Skills",activeTab)}
          ${navBtn("actions","⚡","Actions",activeTab)}
          ${navBtn("log","🕒","Log",activeTab)}
  ...
  function openAddSkillModal() {
    modal("Add Skill", `
      <div class="stack">
        ...
        <div class="stack" style="gap:6px">
          ...
          <input class="input" id="newSkillIcon" placeholder="⭐" />
        </div>
        <div class="stack" style="gap:6px">
    ...
    `, [
      ...
      { label:"Add", primary:true, onClick:(close)=> {
        ...
        const icon = ($("#newSkillIcon")?.value || "").trim() || "⭐";
  ...
  function openEditSkillModal(skillId) {
    ...
    `, [
      ...
      { label:"Save", primary:true, onClick:(close)=> {
        ...
        const icon = ($("#editSkillIcon")?.value || "").trim() || "⭐";
    ...
    function applyReorderUi() {
      ...
      if (reorderOn) {
        listEl.querySelectorAll(".skillWrap").forEach(w => {
          ...
          bar.innerHTML = `
            <button class="iconbtn" data-move-up="${esc(id)}" aria-label="Move up">↑</button>
            <button class="iconbtn" data-move-down="${esc(id)}" aria-label="Move down">↓</button>
...
# Download pako (fallback gzip) - small, avoids embedding giant blobs
$pakoPath = Join-Path $vendorDir "pako.min.js"
if (($Force) -or (-not (Test-Path $pakoPath))) {
  try {
    $url = "https://cdn.jsdelivr.net/npm/pako@2.1.0/dist/pako.min.js"
    Invoke-WebRequest -Uri $url -OutFile $pakoPath -UseBasicParsing | Out-Null
  } catch {
    Write-Host "⚠ Could not download pako. Sync codes will require CompressionStream support." -ForegroundColor Yellow
    Write-Utf8NoBom $pakoPath "/* pako download failed. Sync code fallback unavailable. */"
  }
}

Write-Host ""
Write-Host "✅ Skill Tracker created at: $Target" -ForegroundColor Green
Write-Host "Open locally: $Target/index.html" -ForegroundColor Cyan
Write-Host "On your site: /$Target/" -ForegroundColor Cyan
Write-Host ""
Write-Host "Tip: add a link on your main index.html to: $Target/" -ForegroundColor DarkGray
